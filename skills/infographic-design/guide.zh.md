...
在這之下有一道只適用 SVG 的格式建構前置檢查，執行方式是 `python scripts/check.py out.svg --bg "<畫布色>" --pad <卡片留白>`。
它只有 XML、失效參照與未定義 CSS 變數會 hard fail，文字適配、對比度、畫布邊界與字級則是來源層估算，交由成品檢查確認：

- `check_contrast.py`——從解析到的填色與圖形估算 WCAG 對比度。
- `check_text_fit.py`——估算文字是否超出方塊或畫布。

hard fail 要先處理，再進入渲染；所有版面診斷都要在成品中確認。
最終會不會傷害讀者，由 `visual-output-qa` 檢查實際成品後判定，不直接相信來源估算。
若前置檢查回報問題，修正順序是：先刪字、再縮小字級，最後才考慮放大方塊（靠放大畫布並維持格線間距）。
