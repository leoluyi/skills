...
**放置判準用 branch 測試**(見 writing-great-skills):每條分支都需要的寫進
SKILL.md,只有部分分支走到的推到 reference 後面用 context pointer 指。本
skill 的分支是一般解釋圖、學習回顧、既有圖評估;同一層裡的概念用 co-location
把定義、規則、例外收在同一標題下。

踩過的坑:好規則寫進特例檔,只在那個特例開火。加東西到具名圖表類型之前,
逐條問「這條真的只有這個分支會用嗎」—— 通常只有一兩句是。
...
9. **兩個 reference 打架時,白紙黑字調解。** 例外綁在**目的與出處**上,
   ...
   「某某風格允許」不行。
10. **正面陳述目標行為,讓被禁的那個選項根本不被提起**(negation,見
    writing-great-skills)。具體例:純禁令「照被教的順序」在真實對話迂迴時
    會被讀成「抄逐字稿」,而正面版「依概念相依鏈排序,每節踩在前一節的
    a-ha 上」直接告訴 agent 遇到岔路怎麼辦。

---

## 4. leading word:dial 與 home

本 skill 兩個運作中的 leading word 是 **dial**(旋鈕拉動)與 **home**(旋鈕的
固定原位:abstract / functional / dense)。dial 把「要不要加比喻」從品味變成有
...
評測遵循官方 skill-creator 的標準,本節只記本 skill 專屬的部分;流程細節
需要時直接讀 skill-creator。
...
1. **`expectations` 只寫客觀可驗的陳述**,主觀產出走 `judged-cases.md`。
   對本 skill 而言客觀的是長條是否從零起、payload 是否在箭頭上、對比是否
...
## 10. references 的否定句改寫 (0.11.2)

2026-08-01。`SKILL.md` 的否定詞早前已從 35 降到 6,這一輪處理 `references/`。改的是三個
風格與版面檔(`bytebytego-style.md`、`color-typography.md`、`charts.md`),
`references/` 全樹從 147 降到 131 個 token。

**沒有動 `svg-construction.md` 與 `learn-loop-viz.md`**,即使它們的 token 數最高(39、20)。
那裡的禁令多半是硬技術護欄——會讓 render 壞掉的屬性與構造——而禁令正該用在這種地方,
條件是同時寫明該怎麼做。為了降 token 數把它們改成正面陳述,會把「這樣寫會壞」稀釋成
「建議這樣寫」。

留下來的否定詞也不全是待辦。三個風格檔剩下的多數是對比式定義(「是 X,不是 Y」——正面在前,
