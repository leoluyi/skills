...
**第二階段**在你確認之後才動作：把一份簡短的決策紀錄（決定了什麼、為什麼）加上六個元素寫成一份 goal 檔案：
...
這個拆法是刻意的。goal 的條件是一個判定式，不是裝 context 的容器——goal 機制每一輪都會重讀那段文字來判斷做完了沒，塞一整頁散文進去只會讓判斷一輪比一輪糊（而且在終端機裡貼起來很痛苦）。長的內容留在檔案裡，靠檔案提及帶進去。停損回合數是必填的——goal 會一直跑到條件達成為止，如果條件不可達（測試不穩定、本來就壞掉的測試），沒有停損就會一路空轉燒 token。如果你的 agent 沒有 goal 指令、或不支援檔案提及語法，那就直接把檔案全文當成 prompt。
