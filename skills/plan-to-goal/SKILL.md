...
name: plan-to-goal
description: >-
  把既有粗略計畫補成可自動執行的 goal spec，包含完成條件、限制與停止條件。
  Turn an existing rough plan into a bounded goal spec for autonomous execution.
app-description: >-
  把既有粗略計畫補成可自動執行的 goal spec。
  Turn an existing rough plan into a bounded autonomous goal.
version: 2.1.0
license: MIT
