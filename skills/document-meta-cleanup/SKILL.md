...
- source references and traceability, including RFP section numbers;
...
Collect the facts that carry meaning: numbers, dates, names, technical terms, subject versions, RFP citations, scope boundaries, decisions, unresolved items, commitments, partner requests, owners, source quotations, and links.
...
Scan the five candidate classes and apply the subject test.
...
The artifact is clean when no remaining sentence talks about its own construction, reading path, page role, or commissioning context unless that reference performs a named navigation, evidence, instructional, or audit function.
