...
Four axes. Each links to the exhaustive table in [`references/term-table.md`](references/term-table.md); the high-frequency core lives inline below so a quick pass needs no lookup.

### A. 詞彙替換 (vocabulary)

The largest category: everyday and technical nouns/verbs where Taiwan and the mainland simply use different words. High-frequency core (full IT + 生活 tables in the reference file):

| 陸用語 | 台灣正體 | Carve-out（保留原詞的語境） |
|---|---|---|
| 視頻 | 影片 | — |
| 屏幕 | 螢幕 | — |
| 軟件 / 硬件 | 軟體 / 硬體 | — |
| 網絡 | 網路 | — |
| 信息 | 資訊(泛指)／訊息(一則訊息) | 資訊理論等固定譯名 |
...
Mainland tech-industry and corporate jargon. Most of these say little on their own — the fix is usually to **name the specific thing**, the same move `avoid-ai-writing-zh` applies to 空話. Core set (full list in the reference file):

| 陸用語 | 台灣講法／改法 |
|---|---|
| 賦能 | 給…能力／支援／強化(講清楚賦予什麼能力) |
| 抓手 | 施力點／著力點／切入點 |
| 對齊 / 拉齊 / 對齊顆粒度 | 取得共識／統一認知／校準 |
| 顆粒度 | 細緻度／精細程度／層級 |
| 閉環 | 完整流程／形成循環／有始有終 |
| 落地 | 實作／導入／上線／落實 |
| 復盤 | 檢討／事後回顧 |
...
Any Simplified codepoint sitting in otherwise-Traditional text is a near-definitive sign of a mainland source or a sloppy 簡→繁 conversion. **P0 flag.** Convert to 正體 and, when the mapping is one-to-many, pick by context.

High-frequency leaks: 为→為 · 这→這 · 个→個 · 们→們 · 时→時 · 会→會 · 说→說 · 对→對 · 长→長 · 东→東 · 车→車 · 门→門 · 问→問 · 网→網 · 软→軟 · 数→數 · 据→據 · 实→實 · 学→學 · 国→國 · 电→電 · 脑→腦 · 应→應 · 让→讓 · 变→變 · 还→還 · 与→與 · 专→專 · 业→業 · 万→萬 · 亿→億 · 广→廣 · 严→嚴 · 两→兩 · 丽→麗 · 处→處 · 户→戶 · 优→優 · 显→顯 · 组→組 · 结→結 · 经→經 · 统→統 · 计→計 · 报→報 · 认→認 · 识→識 · 论→論 · 设→設 · 备→備.

**One-to-many — disambiguate by context** (getting this wrong is worse than leaving it): 发→發(出發)／髮(頭髮) · 面→面(表面)／麵(食物) · 里→里(公里)／裡(裡面) · 松→松(松樹)／鬆(放鬆) · 谷→谷(山谷)／穀(穀物) · 干→干(干涉)／乾(乾燥)／幹(幹部) · 制→制(制度)／製(製造) · 划→划(划船)／劃(規劃) · 表→表(表格)／錶(手錶) · 后→后(皇后)／後(後面) · 云→云(古語)／雲(雲端) · 系→系(科系)／係(關係)／繫(聯繫).

Full mapping and disambiguation notes in [`references/term-table.md`](references/term-table.md).
...
**音譯（外國專名）** — Taiwan and the mainland transliterate foreign names differently. Core:
...
Prioritize by how loudly a term signals a mainland source. Use P0+P1 for a quick pass; full audit covers all three.

### P0 — 明顯陸源（一定改）
- **簡體字殘留** — any leaked Simplified character
- Unambiguous 陸用語詞彙 — 視頻、軟件、屏幕、網絡、打印、缺省、默認、服務器、內存、激活、卸載、土豆
- 音譯專名 — 奧巴馬、悉尼、硅谷、芯片、激光、意大利

### P1 — 互聯網黑話與語法（發布前改）
- 職場／互聯網黑話 — 賦能、抓手、對齊顆粒度、閉環、落地、賽道、內卷、對標、降本增效
- 語法習慣 — 給到、通過(表手段)、進行…的動作、存在…的問題、V+化 堆疊

### P2 — 邊界與已在地化（有時間再改）
- Taiwan-adopted borrowings used heavily — 優化(→最佳化,正式語境)、網紅、視窗 vs 窗口
- 口語 — 立馬、靠譜、特…／賊…
- Context-sensitive vocabulary where the sense is genuinely ambiguous
