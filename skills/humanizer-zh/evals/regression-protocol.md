...
- **保護類**（`no-false-positive-*`、`preserves-*`、`does-not-*`、
  `*-not-*` 這族語意）：**確認誤殺 0**。本 skill 的 ethos 是寧可漏標也別誤傷
  真人，保護類退步比命中類漏抓嚴重。
...
**一輪的分數不是判定。** 同一份 skill 文字連跑七輪，新臂保護類紅數落在 0 到 3
之間，而且**每輪紅的是不同的列**——七輪合計 8 列曾經紅過，沒有一列穩定紅。單輪
的絕對零門檻在這個變異度下是擲骰：抽到 0 就過，抽到 2 就擋，兩者是同一份文字。

所以門檻改架在重複性上：**真缺陷會重複出現，抽樣噪音不會。**

- 最少 **3 輪**。兩輪只能說「這兩輪一致」或「不一致」，前者跟同一個噪音抽兩次
  無法區分。
- 某條保護列在 **2 輪以上**紅，才算**確認誤殺**；確認誤殺必須為 0。只紅一輪的
  列記為未確認，是下一輪要去分辨的對象，不擋出貨。
- 另加兩道防止「不重複」變成漏洞的護欄：新臂保護類**每輪平均紅數不得高於
  baseline**（否則每輪紅三條不同的列就永遠沒有重複），命中類同樣比平均。

各輪必須是同一個樣本：同一份 skill 文字（比對 `new_blob_sha256`，不是 version
字串——沒 bump 的編輯不會改變 version）、同一個 baseline、同一份判分準則、同一組
runner／grader 模型。任一項不同即硬錯，不是警告。

```
tools/run-case --aggregate skills/<skill>/evals/results-*.json
```

partial（`--ids`）產出不能當一輪：沒跑到的列在聚合時讀起來像通過。
...
兩件事是這個工具組的常駐限制，不是待辦事項——它們不會被某一次修補關掉，讀任何一份
`results-*.md` 都要先記得：

- **`corpus.md` 已飽和：它偵測退步，不衡量進步。** 1.5.0 在上面拿 89/89（保護 52/52、
  命中 37/37），2.0.0 同樣 89/89。當回歸護欄繼續跑，但**一個持平的 89/89 不是任何東西的
  證據**。往後任何「我們變好了」的主張，需要的是新的、未飽和的素材，不是這份 fixture 的分數。
- **英文側的證據不獨立。** `references/en-rules.md` 有三組 before/after 是對著 corpus 的
  gold fragment 寫出來的（post-regression 補丁），所以它們產生的英文命中屬於辨認，不是推導。
  **id 51** 是同一個形狀：runner 為自己的 carve-out 辯護時，引的規則例句與測試輸入近乎逐字相同。
  兩者都要等新的、未被用過的英文素材出現才能換成合成材料。相關：2.0.0 那一輪沒有可用的
  **跨家族判分**，所以它的結果只支持「沒有偵測到退步」，永遠不是「我們更好」。
