...
| 基線（泛指 baseline 的比較起點或標準，如「建立基線」「與基線相比」） | 把 baseline 直翻成中文；「基線」沒有說明比較的是數值、版本、目前表現還是對照組，泛用技術／商務文字裡容易顯得生硬 | 保留 `baseline`，或依具體意思改成「基準值」「基準版本」「目前表現」「對照組」；能寫出實際比較對象就直接寫明 | 字型排版、幾何、網路、訊號等已有明確定義的專業術語；正式標準或文件已定義「基線」者保留 |
...
- 收束整段的總評，其所總結的各項就在同段前文逐項寫出時，**不標**——成語在這裡是收束，不是頂替。
- 同段並陳負面判定時（「網頁設計功力不行…推理能力表現不俗…可圈可點！」），帶方向的四字評語屬評測語域：負評在場就是這篇在評而不是在推的證據，換成「不錯」會抹掉方向。
...
判斷型規則（零資訊警句與口號、翻譯腔、過度簡寫、破碎短句堆疊）的定型負面案例對照。遇到相同句型可直接套用下表；句型不同、或不確定是否觸發，仍回 `zh-rules.md` 依各規則判準處理。左欄是要抓的負面案例，中欄標出病灶與對應的規則名稱，右欄是改法。
...
| 這些技能照台灣人實際的寫法寫 | 破碎短句堆疊（繫詞架構被抽掉：句尾「寫法寫」懸空，缺「是…寫成的」句架） | 這些技能是照著台灣人實際的書寫習慣寫成的 |
| 理解後文的干擾機制，需要先具備四個基礎概念 | 破碎短句堆疊（缺連接詞／目的：子句抽成裸動詞片語，讀者先把「理解…」誤讀成主語才回頭修正） | 如果要理解後文的干擾機制，需要先具備四個基礎概念 |
| 資料量超過一百萬列，這個查詢會退回全表掃描 | 破碎短句堆疊（缺連接詞／條件：讀成已發生的事實，而原意是條件） | 資料量一旦超過一百萬列，這個查詢就會退回全表掃描 |
| 連線數沒設上限，尖峰時段直接打爆資料庫 | 破碎短句堆疊（缺連接詞／因果：兩句先讀成並列的兩件事，因果得回頭補） | 連線數沒設上限，所以尖峰時段直接打爆資料庫 |
| 文件寫得很清楚，實際跑起來完全不是那回事 | 破碎短句堆疊（缺連接詞／轉折：前句讀成正面評價，到後句才發現是讓步） | 文件雖然寫得很清楚，實際跑起來卻完全不是那回事 |
| 換掉憑證，所有既有 session 失效 | 破碎短句堆疊（缺連接詞／時序：讀不出「換掉」是條件還是與後句並列的另一個動作） | 換掉憑證之後，所有既有 session 都會失效 |
