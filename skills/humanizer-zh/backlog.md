...
- [x] **`口語化萬能詞`'s new 名詞與短語 form needs eval coverage on both sides.** The rule was
  ...
  **落地 2026-08-02，三處。** 命中側加在 **id 7 自己的 key**（`flags-colloquial-catch-all-phrase`）
  ——「安裝有兩條路」本來就在那個 prompt 裡，也正是 `zh-rules.md` 這條規則自己的對照句，缺的只是
  沒有人把它寫進期望。保護側兩案：**id 76**（`voice: casual`，「那一塊」「擋著」「喊」三個表面
  命中，靠保留欄第一項的語域放行）與 **id 77**（宣告園藝比喻系統的署名文體）。

  **id 77 兩個方向寫在同一案，這是刻意的。** 只驗「宣告過的比喻不被標」證不了 carve-out 的
  範圍——一個把宣告讀成整段豁免的 runner 照樣全過。所以同一段裡放一個宣告沒涵蓋的「兩條路」，
  它仍須被標。carve-out 只有在鄰近一個它不涵蓋的片段仍然命中時才算驗過。

  **這兩案是合成語料，與第 4a(4) 項的判定不衝突，理由要記著**：4a(4) 說的是主張「這是真人會寫的
  東西，不准動」的保護案需要真語料，因為合成句是人味的演出。76、77 主張的不是人味，是**宣告的
  語域與宣告的比喻系統會不會抑制規則**——測的是機制，而宣告本身就是 prompt 的一部分，真語料
  帶不進來那個宣告。

### 6. rewrite mode 的口語時間表達 保真 case — 收工 2026-08-02

- [x] **A 保真 case for colloquial time expressions in rewrite mode.** In the 2026-07-30 run,
  ...
  **落地為 id 78**（同時進 `run-case.json` 的 `rewrite_case_ids`）。案子裡配了兩條真的要改的
  AI 味（「值得注意的是」、「採取…的方式進行…以確保…」），保真列才不會靠「什麼都不做」過關——
  一個原地不動的 runner 應該在命中列失分。

### 7. `破碎短句堆疊` 完全沒有 eval 覆蓋，而它剛長大 — 收工 2026-08-02

- [x] **`evals.json` 對 `破碎短句堆疊` 的 case 數是 0。** 對照：`語體漂移` 落地時配 13 案，
  ...
  **命中側三案落地 2026-08-02**：id 79（推論鏈斷成連續斷言）、id 80（缺連接詞／目的子句抽成
  裸動詞片語）、id 81（繫詞架構被抽掉）。id 81 兼作邊界重切的回歸哨——它的第二列要求該片段
  掛 `破碎短句堆疊` 而非 `過度簡寫`，還在用舊歸屬的 runner 會在那一列失分。

  **三處 label 逐處查過，都跟著邊界搬了**：`evals.json` 剩下的兩處 `過度簡寫` 都是 carve-out
  引用（id 8 的通行簡稱、id 76 的「不得改標他條」）而非歸屬；`corpus.md` 三處全是 `ok` 列的
  放行理由（公文句架 ×2、真人筆誤 ×1），不是命中歸屬；`zh-phrase-rules.md` 的定型表只剩
  「術語不誤傷」掛 `過度簡寫`（動詞裸用），其餘六列已全數掛 `破碎短句堆疊`。沒有一處還停在
  舊邊界。

  **保護側卡住了，而卡住的原因是 references 內部先打架。** 本項原文指名的保護案句子——
  「資料量超過一百萬列，這個查詢會退回全表掃描」——**已經是 `zh-phrase-rules.md:101` 的定型
  負面案例**，該列判它 `破碎短句堆疊（缺連接詞／條件）`，改法給「資料量一旦超過一百萬列，
  這個查詢就會退回全表掃描」。同一句話，backlog 要它放行、定型表要它被抓。

  這不是筆誤，是規則內部的真空：`zh-rules.md` 的保留欄寫著「中文本來意合，關係從語序一次就讀
  得出來時不補」，而定型表給了 條件／因果／轉折／時序／目的 五列該抓的例子——**那五列覆蓋的正是
  意合最常出現的全部關係型別**。命中側有五個工作範例，保留側是一句沒有範例的散文。兩者的分界
  無處可讀，所以任何保護案都會落在爭議區。

  **id 82 因此先走沒有爭議的 carve-out**（摘要與條列重點的壓縮是這些區塊的職責）。

  **分界已裁決 2026-08-02：收窄保留欄，不動定型表。** 五列是工作範例、它們正確；錯的是保留欄把
  抓欄已有的回讀測試復述得太鬆，鬆到讀起來像對全部意合句的無條件豁免。保留欄第一分句改成接回
  回讀測試，並把定型表五種誤讀路徑（讀成已發生的事實、讀成並列的另一件事、讀成正面評價、讀成
  整句的主語）點名為「沒有讀出來」。推導與被否決的另一個選項記在 `design-notes.md`。

  **保護側落地為 id 85**：兩句都是意合、都沒有連接詞，第一句該抓（前半先讀成已發生的事實），
  第二句該放（上週／這週把時序釘死，第一遍就讀對）。把判準讀成「有沒有連接詞」的兩個方向都會
  在這一案上失分。id 82 維持原樣，走的仍是條列 carve-out。作者另補一則 casual 語域的三形態案
  落地為 id 86（裸斷言＋缺條件標記，同時樁住「casual 不是本條的語域分支」與錯字保護）。

  **措辭改了三次才收斂**，兩次退步的病因相同——規則文字裡的枚舉會被讀成完整的判定演算法。
  第三版在列舉末端補上前置條件（四條路徑都以關係詞缺席為前提）。過程與教訓記在 `design-notes.md`。

### 8. `check-labels` 在 main 上連紅三個 commit — 收工 2026-08-02

- [x] **閘紅了，而且不是因為有人寫錯 label。** `1ec104a` 起紅（此前一路綠），`661e402`、
  ...
  **落地 2026-08-02，兩個分歧點都選了「各自長一層」。** `改寫保真` 進 config 新欄位
  `expectation_classes`（不是 `names` 常數）：它是跨規則的期望類別，塞進規則名單等於宣稱
  它是一條規則，而 `resolve()` 也就報不出「這個名字該從哪裡宣告」。`zh-phrase-rules.md`
  以**白名單 regex** 納入（`^## (四字評語)`，不是整檔）：該檔 9 個 header 只有這一個當規則名
  用，整檔納入會讓「台灣用語偏好」這種非規則 header 進 canonical，而精確比對之下，混進去的
  名字會讓別處的錯字變合法。另加一道 overlap 檢查：同一名字不得兩個 namespace 都宣告。

  **驗過它不是靠變寬鬆才綠的**：canonical 65 個名字，`zh-phrase-rules.md` 另外 8 個 header
  逐一驗證皆被拒；把 evals id 60 的 `對應規則` 改成 `改寫保真x` 端到端仍 FAIL。這是
  `CLAUDE.md` 那條「新檢查要對真實過往產物回歸測試才可以擋人」的最小版本——修一個閘之後
  要證明的不是它綠了，是它還會紅。

### 9. `改法` 行外溢：覆蓋量已量出來，痕跡調查卡在儀器 — 2026-08-02
...
- [~] **`語體漂移` 那次外溢是單一觀察，但機制看起來是通則。** 它的 `改法` 給了「降格成條目：
  ...
  **覆蓋問題 2026-08-02 量完了，答案比預期難看，而且它先於痕跡調查成立：**
  ...
  上面原文寫 `對比句式` 4 案，實際計分覆蓋是 3——第 4 處只在散文提及，不進計分。
  ...
  **所以順序反過來了**：不是「拿現有 eval 看痕跡」，是先給 `對讀者說教` 與 `對比句式` 各補
  ...
  屬本輪範圍；但補完就要重跑，所以它和第 5、6、7 項同一批走，不另開。
...
- [x] **正式文件語料的下一個來源：學術計畫。** 作者 2026-08-01 的判斷是學術計畫比政府計畫書
  ...
  **收工 2026-08-02，加了兩份國科會成果報告全文**：H-25（黃慕萱，圖書資訊學，NSC 95-2413-H-002-006）
  換作者，H-26（鄧成連，設計管理／工業設計，NSC88-2411-180-002）同時換作者與學門。H-25 與 H-24
  共用同一個四字語而判定相反——H-24 的「相輔相成」後面沒寫出兩個計畫如何互相援引，H-25 的
  「相輔相成」前面就把兩種指標各自的貢獻寫完了——這一對把判準釘在鄰句的著落上，而不是那四個字。
  H-26 另帶進兩組本檔原本沒有的保護：報告自己在同段宣告的醫學比擬系統，以及兩個真人筆誤
  （「診計診斷」順序顛倒、「進乙步」同音誤植），是本檔第一則以錯字入桶的例子。
  **判定欄待作者複核**——逐字節錄與出處由 agent 負責，每一列判得對不對是品味判斷。
...
- [ ] **`SKILL.md:130` 把 voice 宣告的正向特徵升格成保護清單⑥，於是「短句」變成豁免（id 86）。**
  2026-08-03 補跑單案讀 runner 原始輸出定位到的，取代此前兩個都不成立的說法。runner 把兩個該被
  `破碎短句堆疊` 抓的 span 直接掛進保護清單（`⑥「放心壓」：符合使用者宣告的 casual 口語措辭`、
  `⑥「就別開」：符合 casual 的短句與口語提醒`），「會結束，放心壓」全篇未曾被當作該規則討論過
  ——不是判它不成立，是它沒有進入判定。

  機制在 `:130` 的「declares positive features, those features are 保護清單 item ⑥」，而 Voice
  `casual` 在 `:128` 宣告的特徵字面就含 short sentences。`:128` 本身只是描述，把描述變成豁免的是
  `:130` 的升格條款。**病根是升格條款沒有區分「作者刻意造出來的東西」（宣告過的比喻系統、刻意的
  口語破格）與「任何文字都自動滿足的通用屬性」**；前者是它要保護的，後者讓專抓短句的規則被自己的
  保護清單架空。
