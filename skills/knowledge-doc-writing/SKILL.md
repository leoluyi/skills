...
**HTML edition:** Markdown is the default output and the source of record.
...
After rendering the HTML edition, run the `visual-output-qa` skill against the final browser artifact; it owns cross-format rendered truth principles and the delivery verdict.

**Completion criterion:** the whole document reads plainly on one pass, with established terms kept in English; every list in every section has a lead-in and a wrap-up sentence, with meaningful ordering stated explicitly; there's a one-sentence blockquote opener and a one-paragraph closer; architecture and decisions are rendered in Mermaid and readable on their own; an HTML edition is produced only when the user asked for one, and when it is, the four types stay separated, diagrams are inline SVG, the Markdown edition remains the source of record, and `visual-output-qa` returns `PASS` for the browser delivery profile.
