...
**D. 既有文件改寫**。輸入是一份已存在的文件，任務是修補或重構它，不是憑空重寫。動筆前依序做：

1. **先辨識文件出身**。已經是本 skill 骨架（What/Why＋辯證比較五件套）→ **定點修補**；外來格式（教學文、feature list、其他團隊的文件）→ **整份重構**。這一步排在任何改寫動作之前——判斷錯了，不是做出不必要的重寫，就是做出半套的重構。
2. **主題型態先過一次篩**。本模式只收技術知識文件；輸入若本身是簽呈、會議紀錄、評估報告等行政文件，即使格式再「外來」，也轉交 `formal-doc-structure`，不因為動詞是「改寫」就留在這裡。
3. **對應到四種常見情境（可同時成立）**：重構（外來格式套進骨架）、更新時效（版本、預設值、as-of 日期、已淘汰的替代方案）、併入新素材（新來源併入既有文件，保留仍正確的部分）、品質升級（補齊空洞的 What/Why、補「何時不該採用」、把退化成名詞堆砌的條列升級回論述——僅止於結構層級，句子層級的 AI 味仍交給 avoid-ai-writing-zh，不在這裡動手）。
4. **動筆前先問，不自行假設**：保守改寫（維持原有結構與判斷，只做局部修補）還是從零重產（整份依本 skill 骨架重寫）。每次都問，不因為看起來像哪一種就跳過。
5. **保守改寫時，不重做已成立的判斷**。舊文件裡已經寫對、寫清楚的論述與比較，是文件主幹，照用；不要退回去重新論證或重新排版——那是重構的範圍，不是這裡的工作。對已判定沒問題的段落不動，不因為在改別處就順手改寫它。
6. **時效性事實一律回一手來源重查，不採信舊文件本身的說法**（見「來源可靠性」）：版本、預設值、已淘汰的替代方案都要重新查證，as-of 日期更新為出文件的時點。
7. **交付一份改寫說明**：列出結構變動（章節增刪）、時效性事實更正（舊值→新值＋來源）、內容併入或保留的取捨。沒有這份清單就宣稱改完，等同完稿檢查裡「沒有清單就宣稱通過，視同未檢查」的同一種違規。

四種情境與定點修補／整份重構的判斷細節、改寫說明範例，見 [references/rewrite.md](references/rewrite.md)。
...
- **繁體中文白話、台灣用語、少縮寫**。術語保留英文（API、sidecar、control plane），這是正確的台灣技術寫作慣例；無定譯的專有名詞不生造中文譯名。
...
- **開頭收一個「一句話」blockquote**：一句話定義主題＋文件範圍行＋「更新至 YYYY-MM」。**文末收「一段話總結」**：三行內講完定義、強弱項、決策法則。
- **概念白話講解用三拍節奏**：一句話 → 為什麼重要 → 要注意的坑。坑那一拍常常是最有價值的。
- **表格承重**：邊界判斷用「需求 → 該用的工具 → 是否本職」表；決策框架用「訊號 → 傾向採用／傾向不用」雙欄表。兩三個項目的簡單比較仍用文字寫。
