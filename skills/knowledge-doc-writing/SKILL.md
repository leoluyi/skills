...
| | 學習筆記模式 | 正式文件模式 |
|---|---|---|
| 口吻 | 可用第一人稱、可留疑問 | 客觀陳述，規格手冊風格；陳述主題本身，不對讀者喊話（無第二人稱教練口吻） |
...
1. **語言檢查（實際執行，不是宣稱）** — 以 `avoid-ai-writing-zh` 的 detect 模式對全文產出違規清單，逐項修復後複掃至清零，並在交付說明中回報掃描結果（發現幾處、修了幾處、殘留幾處與理由）。正式與混合模式一併把第二人稱教練口吻（對讀者下判斷或喊話，如「抹掉你哪裡不懂」）視為違規掃除；學習筆記模式放寬第一人稱與自問。可機械檢核的項目先跑腳本：連接用破折號（——）每千字一次為上限（條列「概念名 — 說明」分隔符不計）、grep 慣用詞替換表的 Flag 詞（跳、節奏、編排…）。沒有違規清單就宣稱通過，視同未檢查。
