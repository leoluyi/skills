...
[![Skills](https://img.shields.io/badge/skills-25-6d4aff?style=flat-square)](#技能目錄)
...
25 個技能、7 個分類。每個技能都標了叫用方式：`model-invoked`（任務命中 trigger 時 agent 會自動載入，也能自己指名叫）或 `user-invoked`（只有你能指名叫）。可看[互動式目錄 ↗](https://leoluyi.tw/skills/)，或直接讀任何一份 [`SKILL.md`](skills/)。
...
| **繁中部落格寫作**<br>[`blog-writing-zh`](skills/blog-writing-zh/SKILL.md) | `user-invoked` | 把筆記、演講或一個題目，寫成有立場、有親身經歷、讀起來像真人的繁中部落格文 |
...
| **正式公文結構**<br>[`formal-doc-structure`](skills/formal-doc-structure/SKILL.md) | `user-invoked` | 把粗略需求變成可直接送簽的簽呈、會議紀錄或評估報告，結構跟著讀者的決策需求走 |
| **技術 RFP 撰寫與審查**<br>[`rfp-writing`](skills/rfp-writing/SKILL.md) | `user-invoked` | 站在招標方立場撰寫與審查技術 RFP，砍掉重複章節、附錄灌水與 AI 填充語 |
...
| **開發者技術規格工作流**<br>[`developer-spec-workflow`](skills/developer-spec-workflow/SKILL.md) | `model-invoked` | 把初始需求逐題問清楚，收斂成一份開發者技術規格、一套可執行 sample 與 end-to-end 證據 |
<!-- CATALOG:END -->
...
| `tools/eval validate <name> \| --all` | 純靜態驗證 eval fixture 與可選設定，不呼叫模型。 |
| `tools/eval trigger <name>` | 依 `evals/trigger-queries.json` 檢查 description 呼叫邊界。 |
| `tools/eval quick <name> [--ids ...]` | 對代表案例做快速單臂回歸檢查。 |
| `tools/eval gate <name> --baseline REF` | 使用 baseline cache 的 adaptive blind A/B 回歸閘門。加 `--ids` 可做受限一輪診斷。 |
