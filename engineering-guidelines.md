...
2. **Draft** — `tools/new-skill <kebab-name>` scaffolds `skills/<name>/SKILL.md` plus `skills/<name>/evals/` (`evals.json`, `trigger-queries.json`, and optional `config.json`).
...
- **A single-skill dev tool takes config, never hardcoded paths.** Make participation opt-in through a declaration the skill ships, so a skill that hasn't opted in **skips cleanly** rather than erroring. `tools/check-labels` reads `skills/<name>/evals/label-check.json`; `tools/eval` reads optional `skills/<name>/evals/config.json` and otherwise uses standard defaults. A file a config declares but that is missing stays a hard error.
...
- **Rewording a harness prompt invalidates its baseline.** Include the prompt hash in cache keys and rerun the relevant suite after changing the judging brief.
