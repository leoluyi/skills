...
└── evals/
    ...
    └── score-evals.json  # optional — opt into the tools/score-evals scoring harness
```
...
Every skill is **model-invoked** (agent or user can fire it — the default) or **user-invoked** (only the human typing its name, via `disable-model-invocation: true`). The catalog labels every skill with one of these two, on the card pill and the README table. Which to pick, and how it changes what the `description` is written for, is in [`.agents/invocation.md`](.agents/invocation.md).

**A user-invoked skill takes two declarations, one per harness.** `disable-model-invocation: true` in the frontmatter is a Claude-Code field Codex reads as a no-op (see [Portability](#portability)); the Codex side is closed by `policy.allow_implicit_invocation: false` in `skills/<name>/agents/openai.yaml`, which also carries the `interface.display_name` and `interface.short_description` the Codex skill picker shows. The two are set together or neither — a skill half-declared still auto-fires on the harness that was missed, which is the failure the declaration exists to prevent, and `tools/check-invocation <skill>` is what catches it. A fully declared user-invoked skill has no live trigger surface left in either harness, so [`tools/run-eval`](#test-discipline) is no longer a ship gate for one; the `trigger-queries.json` files those skills already carry are kept as a record of what the description was once tuned against, not as evidence anything still fires.
...
The rules above decide how a skill *sounds*. These decide how it is *shaped*, and are the parts of [`writing-great-skills`](https://github.com/mattpocock/skills/tree/main/skills/productivity/writing-great-skills) this file did not already carry.

- **Anchor a region of behaviour on a leading word.** A compact concept the model already holds from pretraining — *tracer bullets*, *fog of war*, *red* — recruits priors instead of spending tokens to build them, and every later mention reaches for the same behaviour. Hunt the inverse when pruning: a passage naming one quality three different ways is a leading word waiting to be extracted. It pays twice, because a word that also lives in your prompts, docs, and code gives the description something sharper to match on.
- **Know whether a passage is a step or a reference.** Steps are ordered actions; reference is consulted on demand, and a flat peer-set of rules is a legitimate shape rather than a smell. This is a different axis from the body/`references/` split — a skill can be all steps, all reference, or both, at any length.
- **End every step on a completion criterion the model can check.** Not "produce a change list" but "every modified model accounted for" — and make it exhaustive where exhaustiveness is the point. A vague criterion invites the model to declare victory early, and that pull is strongest when the later steps are already in view; when sharpening the criterion doesn't settle it, the pull itself is the signal to split.
- **Co-locate a concept's definition, rules, and exceptions under one heading,** so reading one part brings its neighbours with it. This repo learned it the expensive way — humanizer-zh's carve-outs sat apart from the rules they qualified, so they were not available at the moment the rule fired, and moving them adjacent is what stopped the misfires. Complementary to "put a rule where every affected path passes through" above: that one is about not burying a general rule inside a special case, this one about not scattering a single concept across several.
- **Two reasons to split a skill, and no others.** By invocation, when a piece needs its own trigger surface or another skill must be able to reach it — the price is a description sitting in the window permanently, so the reach has to be worth it. By sequence, when the steps still ahead are what tempt the model to rush the one in front of it.
...
- **A single-skill dev tool takes config, never hardcoded paths.** Make participation opt-in through a declaration the skill ships, so a skill that hasn't opted in **skips cleanly** rather than erroring — an exit-2 on every other skill is what stops a gate from being generalizable. A file the config *does* declare but that is missing stays a hard error: opting in and then losing a rule file is a defect, not an absent feature. `tools/check-labels` reads `skills/<name>/evals/label-check.json` (regex sources for the canonical rule names, plus an optional corpus path) and is run `--all` by `.github/workflows/eval-labels.yml`; the config declares *where names live*, never the names, so there is still no taxonomy manifest to drift. `tools/score-evals` follows the same contract through `skills/<name>/evals/score-evals.json`, and adds the reason the declaration has to be explicit rather than inferred: it declares which cases run in rewrite mode, and inferring that from the prompt text silently moved the denominator the ship gate is measured against.
- **An invariant of what a skill *is* gets a repo-wide gate instead, with no opt-in.** The opt-in contract above is for gates a skill grows into; a rule every skill is already subject to has nothing to opt into, and letting one decline is the drift. `tools/check-invocation` is the one of these: it reads `SKILL.md` and `agents/openai.yaml` for every skill, and fails a skill that ships neither declaration or that contradicts itself across the two. It stays narrow on purpose — it never judges which mode a skill *should* be in, and it tolerates unknown keys in a schema this repo does not own, because a gate that fails on a field OpenAI added last week would be wrong more often than the drift it guards. Run `--all` by `.github/workflows/invocation-check.yml`.
- **Opt a skill into a gate only when the gate has something to check against.** `check-labels` resolves a report's rule names against the regex sources a skill declares, so a skill with no rule taxonomy — `infographic-design` has `evals/judged-cases.md` but no canonical rule names — cannot opt in without first inventing one. Inventing a taxonomy to satisfy a gate inverts the order: the taxonomy has to earn its place in the skill first, and the gate follows. Being *able* to opt a skill in is not a reason to.
...
- **Two harnesses, two layers, two obligations.** `tools/run-eval` measures **triggering accuracy** against `trigger-queries.json` — whether the skill fires on the prompts it should and stays quiet on the ones it shouldn't, the two failure classes being **false triggers** and **failed to trigger** — and is what a `description` change needs (and what a fully declared user-invoked skill no longer needs at all; see [Invocation modes](#invocation-modes)). `tools/score-evals` scores the behaviour layer against `evals.json`, and is **opt-in**: a skill joins by shipping `evals/score-evals.json`, and one that hasn't is skipped cleanly rather than failed. Beating the baseline is the requirement; `score-evals` is one mechanization of it, not the only admissible evidence — the parallel-agent comparison below stands on its own. The name split is upstream's, not this repo's invention: Anthropic's `skill-creator` ships `run_eval.py` for the trigger layer too, so `run-eval` reading `trigger-queries.json` rather than `evals.json` is the convention rather than a misnomer.
- **`score-evals` earns its place on three counts, all three or none.** The fixture is large enough that a hand-judged round is the slower path; the output is blind-gradable, so two arms side by side with the labels stripped can be called by someone who never saw the skill; and the skill is still changing, so there is a regression worth guarding. `humanizer-zh` is the one skill here that clears all three, and the harness's declaration classes (`rewrite_case_ids`, `global_rewrite_checks`, `verdict_class`) were shaped around its rewrite corpus. A small fixture declaring those classes is ceremony, not measurement — and a skill nobody is editing has nothing to regress.
...
- **Cross-check a suspicious trigger result on the second harness before calling it a regression.** `tools/run-eval` drives `trigger-queries.json` through Claude Code by default and through Codex with `RUN_EVAL_AGENT=codex`; both are pinned and both refuse to spend a billed API key. When the two disagree case-by-case, the harness is the suspect; when they agree, the triggering change is real. Note the two harnesses do not share a word for what is being measured — Anthropic's docs say a skill *triggers*, OpenAI's say the model performs *implicit invocation* — so name the artifact after the fixture (`trigger-queries.json`) rather than after either vocabulary. Re-running the same CLI and hoping tells you nothing.
## Maintenance
...
- **Rewording a harness prompt invalidates the baseline the same way a model upgrade does.** The judging prompt inside `tools/run-eval` is the instrument, so results recorded before an edit are not comparable with results after it — re-run the suite and treat the new numbers as the new zero, rather than reading the delta as a change in the skill. Its last reword was 2026-08-07, when the role sentence dropped "skill router" for Anthropic's own vocabulary; every trigger result dated before that is a pre-reset reading. The same holds for `tools/score-evals`, where `calibration.json` already pins the thresholds a given prompt-and-model combination produced.
