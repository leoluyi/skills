...
[![Skills](https://img.shields.io/badge/skills-22-6d4aff?style=flat-square)](#skill-catalog)
...
22 skills across 7 categories. Every skill is labelled `model-invoked` (the agent fires it automatically on a matching task, and you can also call it by name) or `user-invoked` (only you can, by name). Browse the [interactive catalog ↗](https://leoluyi.tw/skills/) or read any [`SKILL.md`](skills/) directly.
...
| **Diagram Style**<br>[`diagram-style`](skills/diagram-style/SKILL.md) | `model-invoked` | Restyle an existing architecture or flow diagram while preserving its nodes, connections, groups, and text |
